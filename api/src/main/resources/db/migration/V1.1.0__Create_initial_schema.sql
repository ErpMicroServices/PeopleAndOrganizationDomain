-- Create the initial database schema for People and Organizations Domain
-- This migration creates all core tables and relationships

-- Party Type hierarchy
...
-- Core Party table - foundation for Person and Organization
...
-- Party Identification - external IDs like SSN, EIN, etc.
...
-- Party Names with effective dating
...
-- Party Classification Type hierarchy
...
-- Party Classifications - dynamic categorization
...
-- Party Roles - flexible role assignment
...
-- Party Relationship Type hierarchy
...
-- Party Relationships - typed relationships between parties
...
-- Geographic Boundary Type hierarchy
...
-- Geographic Boundaries - countries, states, cities, etc.
...
-- Geographic Boundary Associations - hierarchical relationships
...
-- Contact Mechanism Type hierarchy
...
-- Contact Mechanisms - polymorphic contact information
...
-- Contact Mechanism Geographic Boundary associations
...
-- Party Contact Mechanism Purpose Type
...
-- Party Contact Mechanism associations
...
-- Communication Event Status Type
...
-- Case (renamed from 'case' to 'kase' to avoid SQL reserved word)
CREATE TABLE IF NOT EXISTS kase
(
    id                  UUID                              DEFAULT uuid_generate_v4(),
    description         TEXT                     NOT NULL
        CONSTRAINT case_description_not_empty CHECK (description <> ''),
...
CREATE INDEX IF NOT EXISTS idx_party_party_type_id ON party (party_type_id);
CREATE INDEX IF NOT EXISTS idx_party_role_party_id ON party_role (party_id);
CREATE INDEX IF NOT EXISTS idx_party_role_party_role_type_id ON party_role (party_role_type_id);
CREATE INDEX IF NOT EXISTS idx_party_relationship_from_party_role_id ON party_relationship (from_party_role_id);
CREATE INDEX IF NOT EXISTS idx_party_relationship_to_party_role_id ON party_relationship (to_party_role_id);
CREATE INDEX IF NOT EXISTS idx_party_contact_mechanism_party_id ON party_contact_mechanism (party_id);
CREATE INDEX IF NOT EXISTS idx_party_contact_mechanism_contact_mechanism_id ON party_contact_mechanism (contact_mechanism_id);
